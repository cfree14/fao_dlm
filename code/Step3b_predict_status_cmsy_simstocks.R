...
# CMSY and BSM analysis (v. November 2016)
# Written by Rainer Froese, Gianpaolo Coro, and Henning Winker
# Modified by Chris Free for FAO data-limited methods comparison analysis
...
# Define directories
datadir1 <- "~/Dropbox/Chris/Rutgers/projects/fao_dlm/data"
datadir <- "~/Dropbox/Chris/Rutgers/projects/fao_dlm/data/cmsy_input"
...
# Read data
id_file <- "simulated_stock_info_key_for_cmsy_froese.csv"
catch_file <- "simulated_stock_catch_time_series_for_cmsy_froese.csv"
cinfo <- read.csv(paste(datadir, id_file, sep="/"), as.is=T) 
cdata <- read.csv(paste(datadir, catch_file, sep="/"), as.is=T)
key <- read.csv(paste(datadir1, "simulated_stocks_for_fao_analysis.csv", sep="/"), as.is=T)
...
# Stocks to analyze
stocks <- subset(key, ar==ar.sel & sigmaC==sigmaC.sel)$stockid
stocks <- "SP_100%_RC_0.6sr_0ar_0.2sc_60yr_6"

# Setup output file
outname <- paste("simstock_cmsy_pred_", comp, "_", ar.sel, "ar_", sigmaC.sel, "sc_1missing", sep="")
outfile <- paste(preddir, paste(outname, ".csv", sep=""), sep="/")
outfile.txt <- paste(preddir, paste(outname, ".txt", sep=""), sep="/")
...
# Use Full Schaefer model?
# TRUE if lots of abundance data; FALSE if not much abundance data
FullSchaefer <- F 

# Setup parallel processing
# 3 chains in JAGS if more than 2 cores are available
n.cores <- detectCores()
...
cat("-------------------------------------------\n")
cat("Parallel processing will use",n.cores,"cores\n")
...
if(write.output==T){
  outheaders = data.frame("Group","Region", "Subregion","Name","SciName","Stock","start.yr","end.yr","btype",
   ...
   write.table(outheaders,file=outfile, append = F, sep=",",row.names=F,col.names=F)
}
...
for(stock in stocks){
  ...
  if(start.yr < cdata$yr[cdata$Stock==stock][1]){
    cat("ERROR: start year in ID file before first year in catch file\n")
    return (NA)
  }
  
  # Extract year data
  yr <- as.numeric(cdata$yr[cdata$Stock==stock & cdata$yr >= start.yr & cdata$yr <= end.yr])
  nyr <- length(yr)
  if (length(yr)==0){
  ...
  ct.raw <- as.numeric(cdata$ct[cdata$Stock==stock & cdata$yr >= start.yr & cdata$yr <= end.yr])/1000  ## assumes that catch is given in tonnes, transforms to '000 tonnes
  if(is.na(mean(ct.raw))){
    cat("ERROR: Missing value in Catch data; fill or interpolate\n")  
  }
  ct <- ma(ct.raw)
  
  # Extract biomass/cpue data
  if(btype=="biomass" | btype=="CPUE" ) {
    bt <- as.numeric(cdata$bt[cdata$Stock==stock & cdata$yr >= start.yr & cdata$yr <= end.yr])/1000  ## assumes that biomass is in tonnes, transforms to '000 tonnes
  }else{
    bt <- NA
  }
  ...
  # Determine initial ranges
  # for parameters and biomass
  ##########################################
...
if (save.plots==TRUE) 
{
  jpgname<-paste(stock,"_AN.jpeg",sep="")
  jpgfile<- paste(plotdir, jpgname, sep="/")
...
{
  jpgname<-paste(stock,"_MAN.jpeg",sep="")
  jpgfile<-paste(plotdir, jpgname, sep="/")
  dev.copy(jpeg,jpgfile,
           width = 1024, 
           height = 768, 
...
elapsed.min <- difftime(end.time, start.time, units="mins")
elapsed.hour <- difftime(end.time, start.time, units="hours")
min.per.stock <- as.numeric(elapsed.min) / length(stocks)
stocks.per.hour <- length(stocks) / as.numeric(elapsed.hour)
